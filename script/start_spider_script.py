...
async def handle_login(phone_no, pool):
    ...
    async with async_playwright() as p:
        ...
        if await page.query_selector('div.account-center-qrcode-title'):
            ...
            switch_button_bounding_box = await switch_button.bounding_box()
            if switch_button_bounding_box:
                # 计算元素中心点坐标
                element_center_x = switch_button_bounding_box['x'] + switch_button_bounding_box['width'] / 2
                element_center_y = switch_button_bounding_box['y'] + switch_button_bounding_box['height'] / 2
                # 使用页面的 click 方法点击元素
                await page.mouse.click(element_center_x, element_center_y)
            else:
                print("Element bounding box not available. Cannot perform click.")
        else:
        ...
        try:
            await page.get_by_placeholder("手机号码").click()
            await page.get_by_placeholder("手机号码").fill(phone_no)
...
async def handle_shop_info_crawl(phone_no, pool, storage_path):
    # 爬取店铺名
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False)
        ...
        try:
            ...
            print('无知道了弹窗')
        try:
            await page.locator('text="我已知悉"').click(timeout=2000)
            await page.wait_for_load_state('networkidle')
        except:
            print('无我已知悉弹窗')
        try:
        ...
        try:
            ...
            await page.get_by_role("button", name="我已知悉").click()
        except:
            print('无知悉弹窗')
        try:
...
async def handle_score_info_crawl(shop_id, pool, phone_no, storage_path):
    # 爬取服务体验分
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False)
        context = await browser.new_context(storage_state=storage_path)
...
async def shop_user_assets(shop_id, pool, phone_no, storage_path):
    async with async_playwright() as p:
...
async def shop_orders_info_crawl(shop_id, pool, phone_no, storage_path):
    # 订单详情
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False)
        context = await browser.new_context(storage_state=storage_path)
...
async def shop_not_clear_orders_info_crawl(shop_id, pool, phone_no, storage_path):
    # 待结算订单详情
    # todo 待结算时间选取
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False)
...
async def shop_daily_bill_crawl(shop_id, pool, phone_no, storage_path):
    # 待结算订单详情
    # todo 是否需要翻页
...
async def shop_monthly_bill_crawl(shop_id, pool, phone_no, storage_path):
    # 待结算订单详情
